...
class DenoisingDiffusionTrainer(_BaseTrainer):
    def __init__(self, *args, **kwargs):
        super().__init__(**kwargs)
        self.model_name = 'Denoising_Diffusion'
    ...
    def _train(self, batch, step):
        ...
        self.opt.zero_grad()
        loss = self.diffusion_model(u, labels=u_obs)
        loss.backward()
        self.opt.step()
    ...
    def _test(self, batch, step, mode):
        ...
        samples = self.diffusion_model.sample(self.batch_size, labels=u_obs)
    ...
    def sampling(self, *args, **kwargs):
        ...
        if self.use_ddib and u is not None:
            ...
            with torch.no_grad():
                samples = self.diffusion_model.style_transfer(x=u, labels=obs)
            ...
            obs = self._preprocess(obs)
            with torch.no_grad():
                samples = self.diffusion_model.sample(self.batch_size, labels=obs)
